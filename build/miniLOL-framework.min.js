...
Class.create({parseFromString:function(c){var d=new AciveXObject("Microsoft.XMLDOM");d.async="false";d.loadXML(c);return d}});if(!window.XMLSerializer)window.XMLSerializer=Class.create({serializeToString:function(c){return c.xml}})}})();Object.extend(Function,function(){return{parse:function(a){matches=a.match(/^function\s*\((.*?)\)[\s\n]*\{([\s\S]*)\}[\s\n]*/m);if(!matches)return null;a=matches[1].split(/\s*,\s*/);return new Function(a,matches[2])}}}());Object.extend(Function.prototype,function(){return{clone:function(){return Function.parse(this.toString())}}}());
...
miniLOL.History={interval:0.15,initialize:function(){miniLOL.History.current=window.location.hash;if(Prototype.Browser.Opera&&history.navigationMode)history.navigationMode="compatible";Event.observe(document,":url.change",function(a){miniLOL.History.current=a.memo});miniLOL.History.Initializers.get().call()},reset:function(a,b){if(Object.isNumber(a))miniLOL.History.interval=a;Object.isUndefined(miniLOL.History.timer)||clearInterval(miniLOL.History.timer);miniLOL.History.timer=setInterval(b,miniLOL.History.interval*
1E3)},Initializers:{get:function(){return"onhashchange"in window&&!(Prototype.Browser.InternetExplorer&&Prototype.Browser.Version==7)?miniLOL.History.Initializers.Default:Prototype.Browser.InternetExplorer?miniLOL.History.Initializers.InternetExplorer:miniLOL.History.Initializers.Unsupported},Default:function(){Event.observe(window,"hashchange",function(){Event.fire(document,":url.change",Prototype.Browser.Mozilla?window.location.hash.replace(/^#/,""):decodeURIComponent(window.location.hash.replace(/^#/,
"")))})},Unsupported:function(){document.observe("dom:loaded",function(){miniLOL.History.reset(miniLOL.History.interval,miniLOL.History.Checkers.Default)})},InternetExplorer:function(){document.observe("dom:loaded",function(){miniLOL.History.IE={check:function(){$("__miniLOL.History")||$(document.body).insert({top:miniLOL.History.IE.element})},put:function(b){miniLOL.History.IE.check();var c=miniLOL.History.IE.element.contentWindow.document;c.open();c.close();c.location.hash=encodeURIComponent(b)},
get:function(){miniLOL.History.IE.check();return miniLOL.History.IE.element.contentWindow.document.location.hash},element:new Element("iframe",{id:"__miniLOL.History",style:"display: none !important; z-index: -9001 !important;",src:"javascript:false;"})};var a=miniLOL.History.current;$(document.body).insert({top:miniLOL.History.IE.element});miniLOL.History.IE.put(a);miniLOL.History.reset(miniLOL.History.interval,miniLOL.History.Checkers.InternetExplorer)})}},Checkers:{Default:function(){if(miniLOL.History.current!=
window.location.hash)Event.fire(document,":url.change",Prototype.Browser.Mozilla?window.location.hash.replace(/^#/,""):decodeURIComponent(window.location.hash.replace(/^#/,"")))},InternetExplorer:function(){var a={iframe:miniLOL.History.IE.get(),actual:window.location.hash,current:miniLOL.History.current};if(a.actual!=a.iframe){if(a.actual!=a.current)window.location.hash=a=miniLOL.History.current=a.iframe;else{a=miniLOL.History.current=a.actual;miniLOL.History.IE.put(a)}Event.fire(document,":url.change",
a)}}}};miniLOL.History.initialize();
