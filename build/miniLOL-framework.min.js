...
(function(){if(Prototype.Browser.Gecko)Prototype.Browser.Mozilla=true;if(navigator.userAgent.match(/Chrome/))Prototype.Browser.Chrome=true;if(navigator.userAgent.match(/Safari/)&&!Prototype.Browser.Chrome)Prototype.Browser.Safari=true;if(Prototype.Browser.Mozilla||Prototype.Browser.Opera)Prototype.Browser.Good=true;else Prototype.Browser.Bad=true;Prototype.Browser.Name=window.navigator.appName;var a=window.navigator.appVersion;try{if(Prototype.Browser.IE)Prototype.Browser.Version=parseFloat(a.match(/MSIE ([^;]*)/)[1]);
else if(Prototype.Browser.Mozilla)Prototype.Browser.Version=parseFloat(a);else throw null;}catch(b){Prototype.Browser.Version=0}if(Prototype.Browser.IE)Error.prototype.toString=function(){return"#{name}: #{description}<br/><br/>#{stack}".interpolate({name:this.name,description:this.description,stack:(this.stack||"").replace(/\n/g,"<br/>")})};else if(Prototype.Browser.Opera)Error.prototype.toString=function(){return"#{name}: #{message}".interpolate(this)};else if(Prototype.Browser.Gecko)Error.prototype.toString=
function(){return"#{name}: #{message}<br/><br/>#{stack}".interpolate({name:this.name,message:this.message,stack:this.stack.replace(/\n/g,"<br/>")})};else if(Prototype.Browser.Chrome||Prototype.Browser.Safari)Error.prototype.toString=function(){return"#{name}: #{message}<br/><br/>#{stack}".interpolate({name:this.name,message:this.message,stack:this.stack.replace(/\n/g,"<br/>")})};if(Prototype.Browser.IE){if(!window.DOMParser)window.DOMParser=Class.create({parseFromString:function(c){var d=new AciveXObject("Microsoft.XMLDOM");
d.async="false";d.loadXML(c);return d}});if(!window.XMLSerializer)window.XMLSerializer=Class.create({serializeToString:function(c){return c.xml}})}})();Object.extend(Function,function(){return{parse:function(a){matches=a.match(/^function\s*\((.*?)\)[\s\n]*\{([\s\S]*)\}[\s\n]*/m);if(!matches)return null;a=matches[1].split(/\s*,\s*/);return new Function(a,matches[2])}}}());Object.extend(Function.prototype,function(){return{clone:function(){return Function.parse(this.toString())}}}());
Object.extend(Object,function(){var a=Object.isFunction(Object.defineProperty)?Object.defineProperty:function(d,e,f){Object.isFunction(f.get)&&Object.isFunction(d.__defineGetter__)&&d.__defineGetter__(e,f.get);Object.isFunction(f.set)&&Object.isFunction(d.__defineSetter__)&&d.__defineSetter__(e,f.set)};if(Object.isFunction(Object.defineProperties))var b=Object.defineProperties;else Object.defineProperties=function(d,e){for(var f in e)Object.defineProperty(d,f,e[f])};if(Object.isFunction(Object.create))var c=
Object.create;else Object.create=function(d,e){var f=Object(d);Object.defineProperties(f,e);return f};return{isBoolean:function(d){return typeof d=="boolean"||d.constructor===Boolean},isObject:function(d){return typeof d=="object"},isDocument:function(d){return d.toString().include("Document")},isXML:function(d){if(typeof d!=="object")return false;d=d.ownerDocument||d;if(!d.documentElement)return false;return d.documentElement.nodeName!="HTML"},fromAttributes:function(d){for(var e={},f=0;f<d.length;f++)e[d.item(f).nodeName]=
d.item(f).nodeName;return e},toQueryString:function(d){var e="";for(var f in d)e+="#{name}=#{value}&".interpolate({name:f,value:d[f]});return e.substr(0,e.length-1)},defineProperty:a,defineProperties:b,create:c}}());Object.extend(String,function(){return{fromAttributes:function(a){for(var b="",c=0;c<a.length;c++)b+='#{name}="#{value}" '.interpolate({name:a.item(c).nodeName,value:a.item(c).nodeValue});return b},fromXML:function(a){if(!Object.isXML(a))return false;return(new XMLSerializer).serializeToString(a)}}}());
Object.extend(String.prototype,function(){var a=window.encodeURI,b=window.decodeURI,c=window.encodeURIComponent,d=window.decodeURIComponent;return{toQueryParams:function(){var e={},f=this.match(/[?#](.*)$/);if(!f)return e;f=f[1].split(/&/);for(var g=0;g<f.length;g++){var h=f[g].split(/=/),i=h[0].decodeURIComponent();e[i]=h[1]?h[1].decodeURIComponent():true}return e},toXML:function(){return(new DOMParser).parseFromString(this,"text/xml")},isURL:function(){var e=this.match(/^mailto:([\w.%+-]+@[\w.]+\.[A-Za-z]{2,4})$/);
if(e)return{protocol:"mailto",uri:e[1]};e=this.match(/^(\w+):(\/\/.+?(:\d)?)(\/)?/);if(!e)return false;return{protocol:e[1],uri:e[2]}},blank:function(){return this==0},getHashFragment:function(){var e=this.match(/(#.*)$/);return e?e[1]:""},encodeURI:function(){return a(this)},decodeURI:function(){return b(this)},encodeURIComponent:function(){return c(this)},decodeURIComponent:function(){return d(this)}}}());
window.Element.addMethods(function(){return{load:function(a,b){if(b&&!Object.isUndefined(b.frequency))new Ajax.PeriodicalUpdater(this,a,b);else new Ajax.Updater(this,a,b)},xpath:Prototype.Browser.IE?function(a,b){if(Object.isUndefined(b)){b=a;a=this}for(var c=[],d=a.selectNodes(b),e=0;e<d.length;e++)c.push(d.item(e));return c}:function(a,b){if(Object.isUndefined(b)){b=a;a=this}for(var c=[],d=(a.ownerDocument||a).evaluate(b,a,null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null),e=0;e<d.snapshotLength;e++)c.push(d.snapshotItem(e));
return c},select:function(a,b){if(Object.isUndefined(b)){b=a;a=this}return Prototype.Selector.select(b,a)},getTextDescendants:function(a){function b(d){for(d=d.firstChild;d;){Node.TEXT_NODE==d.nodeType&&c.push(d);Object.isElement(d)&&b(d);d=d.nextSibling}}a=a||this;var c=[];b(a);return c},getFirstText:function(a){a=a||this;var b="";if(Object.isElement(a))a=$A(a.childNodes);else Object.isArray(a)||(a=$A(a));a.each(function(c){switch(c.nodeType){case Node.ELEMENT_NODE:throw $break;case Node.CDATA_SECTION_NODE:case Node.TEXT_NODE:if(!c.nodeValue.blank()){b=
c.nodeValue.strip();throw $break;}break}});return b},toObject:function(a){a=a||this;var b={};if(!Object.isElement(a)&&!Object.isDocument(a))return b;$A(a.childNodes).each(function(c){if(c.nodeType==Node.ELEMENT_NODE)if(c.getElementsByTagName("*").length==0){var d="";$A(c.childNodes).each(function(e){e.nodeType!=Node.CDATA_SECTION_NODE&&e.nodeType!=Node.TEXT_NODE||e.nodeValue.blank()||(d+=e.nodeValue)});b[c.nodeName]=d}else b[c.nodeName]=Element.toObject(c)});return b}}}());
if(!Object.isObject(window.miniLOL))window.miniLOL={error:Prototype.emptyFunction};
miniLOL.utils={exists:function(a){var b=false;new Ajax.Request(a,{method:"head",asynchronous:false,onSuccess:function(){b=true}});return b},execute:function(a){var b,c;new Ajax.Request(a,{method:"get",asynchronous:false,evalJS:false,onSuccess:function(d){try{b=window.eval(d.responseText)}catch(e){c=e;c.fileName=a;c.lineNumber-=5}},onFailure:function(d){c=Error("Failed to retrieve `#{file}` (#{status} - #{statusText}).".interpolate({file:a,status:d.status,statusText:d.statusText}));c.fileName=a;c.lineNumber=
...
miniLOL.History={interval:0.2,initialize:function(){miniLOL.History.current=window.location.hash;Event.observe(document,":url.change",function(a){miniLOL.History.current=a.memo});miniLOL.History.Initializers.get().call()},reset:function(a,b){if(Object.isNumber(a))miniLOL.History.interval=a;Object.isUndefined(miniLOL.History.timer)||clearInterval(miniLOL.History.timer);miniLOL.History.timer=setInterval(b,miniLOL.History.interval*1E3)},Initializers:{get:function(){return"onhashchange"in window&&!(Prototype.Browser.IE&&
Prototype.Browser.Version==7)?miniLOL.History.Initializers.Default:miniLOL.History.Initializers.Unsupported},Default:function(){Event.observe(window,"hashchange",function(){Event.fire(document,":url.change",Prototype.Browser.Mozilla?window.location.hash.replace(/^#/,""):decodeURIComponent(window.location.hash.replace(/^#/,"")))})},Unsupported:function(){document.observe("dom:loaded",function(){miniLOL.History.reset(miniLOL.History.interval,miniLOL.History.Checkers.get())})}},Checkers:{get:function(){return Prototype.Browser.IE?
miniLOL.History.Checkers.InternetExplorer:miniLOL.History.Checkers.Default},Default:function(){if(miniLOL.History.current!=window.location.hash)Event.fire(document,":url.change",Prototype.Browser.Mozilla?window.location.hash.replace(/^#/,""):decodeURIComponent(window.location.hash.replace(/^#/,"")))},InternetExplorer:function(){miniLOL.History.current!=window.location.hash&&Event.fire(document,":url.change",decodeURIComponent(window.location.hash.replace(/^#/,"")))}}};miniLOL.History.initialize();
...
miniLOL.Cookie={get:function(a,b){b=miniLOL.Cookie.options(b);var c=window.document.cookie.match(RegExp.escape(a.encodeURIComponent())+"=([^;]*)","g");if(c){var d=[];$A(c).each(function(e){e=e.match(/^.*?=(.*)$/)[1].decodeURIComponent();d.push(b.raw?e:miniLOL.JSON.unserialize(e)||e)});if(d.length==1)d=d[0];return d}},set:function(a,b,c){c=miniLOL.Cookie.options(c);c.raw||(b=miniLOL.JSON.serialize(b)||b);window.document.cookie=miniLOL.Cookie.encode(a,b,c)},remove:function(a,b){window.document.cookie=
miniLOL.Cookie.encode(a,"",Object.extend(miniLOL.Cookie.options(b),{expires:new Date(0)}))},clear:function(){miniLOL.Cookie.keys().each(function(a){miniLOL.Cookie.remove(a)})},keys:function(){var a=[];$A(window.document.cookie.split(/; /)).each(function(b){b=b.split(/=/);b[1]&&a.push(b[0])});return a.uniq()},encode:function(a,b,c){return"#{key}=#{value}; #{maxAge}#{expires}#{path}#{domain}#{secure}".interpolate({key:a.encodeURIComponent(),value:b.encodeURIComponent(),maxAge:!Object.isUndefined(c.maxAge)?
