...
k)},isElement:function(k){return!!(k&&k.nodeType==1)},isArray:c,isHash:function(k){return k instanceof Hash},isFunction:function(k){return typeof k==="function"},isString:function(k){return g.call(k)===i},isNumber:function(k){return g.call(k)===B},isDate:function(k){return g.call(k)==="[object Date]"},isUndefined:h})})();
...
if(Prototype.Browser.Opera){Element.Methods.getStyle=Element.Methods.getStyle.wrap(function(a,b,d){switch(d){case "height":case "width":if(!Element.visible(b))return null;var f=parseInt(a(b,d),10);if(f!==b["offset"+d.capitalize()])return f+"px";return(d==="height"?["border-top-width","padding-top","padding-bottom","border-bottom-width"]:["border-left-width","padding-left","padding-right","border-right-width"]).inject(f,function(j,n){var c=a(b,n);return c===null?j:j-parseInt(c,10)})+"px";default:return a(b,
...
"none")return a["offset"+b.capitalize()]+"px";return null}return d};Element.Methods.setOpacity=function(a,b){a=$(a);var d=a.currentStyle;if(d&&!d.hasLayout||!d&&a.style.zoom=="normal")a.style.zoom=1;d=a.getStyle("filter");var f=a.style;if(b==1||b===""){(d=d.replace(/alpha\([^\)]*\)/gi,""))?f.filter=d:f.removeAttribute("filter");return a}else if(b<1.0E-5)b=0;f.filter=d.replace(/alpha\([^\)]*\)/gi,"")+"alpha(opacity="+b*100+")";return a};Element._attributeTranslations=function(){var a="className",b=
"for",d=document.createElement("div");d.setAttribute(a,"x");if(d.className!=="x"){d.setAttribute("class","x");if(d.className==="x")a="class"}d=null;d=document.createElement("label");d.setAttribute(b,"x");if(d.htmlFor!=="x"){d.setAttribute("htmlFor","x");if(d.htmlFor==="x")b="htmlFor"}d=null;return{read:{names:{"class":a,className:a,"for":b,htmlFor:b},values:{_getAttr:function(f,j){return f.getAttribute(j)},_getAttr2:function(f,j){return f.getAttribute(j,2)},_getAttrNode:function(f,j){var n=f.getAttributeNode(j);
return n?n.value:""},_getEv:function(){var f=document.createElement("div"),j;f.onclick=Prototype.emptyFunction;f=f.getAttribute("onclick");if(String(f).indexOf("{")>-1)j=function(n,c){c=n.getAttribute(c);if(!c)return null;c=c.toString();c=c.split("{")[1];c=c.split("}")[0];return c.strip()};else if(f==="")j=function(n,c){c=n.getAttribute(c);if(!c)return null;return c.strip()};f=null;return j}(),_flag:function(f,j){return $(f).hasAttribute(j)?j:null},style:function(f){return f.style.cssText.toLowerCase()},
...
if(Element.getStyle(c,"display")!=="inline"&&c.offsetParent)return $(c.offsetParent);for(;(c=c.parentNode)&&c!==document.body;)if(Element.getStyle(c,"position")!=="static")return c.nodeName.toUpperCase()==="HTML"?$(document.body):$(c);return $(document.body)}function d(c){c=$(c);var h=0,g=0;if(c.parentNode){do{h+=c.offsetTop||0;g+=c.offsetLeft||0;c=c.offsetParent}while(c)}return new Element.Offset(g,h)}function f(c){c=$(c);var h=c.getLayout(),g=0,l=0;do{g+=c.offsetTop||0;l+=c.offsetLeft||0;if(c=c.offsetParent){if(c.nodeName.toUpperCase()===
"BODY")break;if(Element.getStyle(c,"position")!=="static")break}}while(c);l-=h.get("margin-top");g-=h.get("margin-left");return new Element.Offset(l,g)}function j(c){return c!==document.body&&!Element.descendantOf(c,document.body)}var n=Prototype.K;if("currentStyle"in document.documentElement)n=function(c){if(!c.currentStyle.hasLayout)c.style.zoom=1;return c};Element.Layout=Class.create(Hash,{initialize:function($super,h,g){$super();this.element=$(h);Element.Layout.PROPERTIES.each(function(l){this._set(l,
null)},this);if(g){this._preComputing=true;this._begin();Element.Layout.PROPERTIES.each(this._compute,this);this._end();this._preComputing=false}},_set:function(c,h){return Hash.prototype.set.call(this,c,h)},set:function(){throw"Properties of Element.Layout are read-only.";},get:function($super,h){var g=$super(h);return g===null?this._compute(h):g},_begin:function(){if(!this._prepared){var c=this.element,h;a:{for(h=c;h&&h.parentNode;){if(h.getStyle("display")==="none"){h=false;break a}h=$(h.parentNode)}h=
true}if(!h){c.store("prototype_original_styles",{position:c.style.position||"",width:c.style.width||"",visibility:c.style.visibility||"",display:c.style.display||""});h=c.getStyle("position");var g=c.getStyle("width"),l=h==="fixed"?document.viewport:c.parentNode;c.setStyle({position:"absolute",visibility:"hidden",display:"block"});var q=c.getStyle("width");h=g&&q===g?a(c,"width",l):h==="absolute"||h==="fixed"?a(c,"width",l):$(c.parentNode).getLayout().get("width")-this.get("margin-left")-this.get("border-left")-
this.get("padding-left")-this.get("padding-right")-this.get("border-right")-this.get("margin-right");c.setStyle({width:h+"px"})}this._prepared=true}},_end:function(){var c=this.element,h=c.retrieve("prototype_original_styles");c.store("prototype_original_styles",null);c.setStyle(h);this._prepared=false},_compute:function(c){var h=Element.Layout.COMPUTATIONS;if(!(c in h))throw"Property not found.";return this._set(c,h[c].call(this,this.element))},toObject:function(){var c=$A(arguments),h={};(c.length===
0?Element.Layout.PROPERTIES:c.join(" ").split(" ")).each(function(g){if(Element.Layout.PROPERTIES.include(g)){var l=this.get(g);if(l!=null)h[g]=l}},this);return h},toHash:function(){var c=this.toObject.apply(this,arguments);return new Hash(c)},toCSS:function(){var c=$A(arguments),h={};(c.length===0?Element.Layout.PROPERTIES:c.join(" ").split(" ")).each(function(g){if(Element.Layout.PROPERTIES.include(g))if(!Element.Layout.COMPOSITE_PROPERTIES.include(g)){var l=this.get(g);if(l!=null){g=g;if(g.include("border"))g+=
"-width";g=g.camelize();h[g]=l+"px"}}},this);return h},inspect:function(){return"#<Element.Layout>"}});Object.extend(Element.Layout,{PROPERTIES:$w("height width top left right bottom border-left border-right border-top border-bottom padding-left padding-right padding-top padding-bottom margin-top margin-bottom margin-left margin-right padding-box-width padding-box-height border-box-width border-box-height margin-box-width margin-box-height"),COMPOSITE_PROPERTIES:$w("padding-box-width padding-box-height margin-box-width margin-box-height border-box-width border-box-height"),
...
"border-bottom":function(c){return a(c,"borderBottomWidth")},"border-left":function(c){return a(c,"borderLeftWidth")},"border-right":function(c){return a(c,"borderRightWidth")},"margin-top":function(c){return a(c,"marginTop")},"margin-bottom":function(c){return a(c,"marginBottom")},"margin-left":function(c){return a(c,"marginLeft")},"margin-right":function(c){return a(c,"marginRight")}}});"getBoundingClientRect"in document.documentElement&&Object.extend(Element.Layout.COMPUTATIONS,{right:function(c){var h=
n(c.getOffsetParent());c=c.getBoundingClientRect();return(h.getBoundingClientRect().right-c.right).round()},bottom:function(c){var h=n(c.getOffsetParent());c=c.getBoundingClientRect();return(h.getBoundingClientRect().bottom-c.bottom).round()}});Element.Offset=Class.create({initialize:function(c,h){this.left=c.round();this.top=h.round();this[0]=this.left;this[1]=this.top},relativeTo:function(c){return new Element.Offset(this.left-c.left,this.top-c.top)},inspect:function(){return"#<Element.Offset left: #{left} top: #{top}>".interpolate(this)},
toString:function(){return"[#{left}, #{top}]".interpolate(this)},toArray:function(){return[this.left,this.top]}});if(Prototype.Browser.IE){b=b.wrap(function(c,h){h=$(h);if(h.nodeType===Node.DOCUMENT_NODE||j(h)||h.nodeName.toUpperCase()==="BODY"||h.nodeName.toUpperCase()==="HTML")return $(document.body);var g=h.getStyle("position");if(g!=="static")return c(h);h.setStyle({position:"relative"});var l=c(h);h.setStyle({position:g});return l});f=f.wrap(function(c,h){h=$(h);if(!h.parentNode)return new Element.Offset(0,
0);var g=h.getStyle("position");if(g!=="static")return c(h);var l=h.getOffsetParent();l&&l.getStyle("position")==="fixed"&&n(l);h.setStyle({position:"relative"});l=c(h);h.setStyle({position:g});return l})}else if(Prototype.Browser.Webkit)d=function(c){c=$(c);var h=0,g=0;do{h+=c.offsetTop||0;g+=c.offsetLeft||0;if(c.offsetParent==document.body)if(Element.getStyle(c,"position")=="absolute")break;c=c.offsetParent}while(c);return new Element.Offset(g,h)};Element.addMethods({getLayout:function(c,h){return new Element.Layout(c,
h)},measure:function(c,h){return $(c).getLayout().get(h)},getDimensions:function(c){c=$(c);var h=Element.getStyle(c,"display");if(h&&h!=="none")return{width:c.offsetWidth,height:c.offsetHeight};h=c.style;h={visibility:h.visibility,position:h.position,display:h.display};var g={visibility:"hidden",display:"block"};if(h.position!=="fixed")g.position="absolute";Element.setStyle(c,g);g={width:c.offsetWidth,height:c.offsetHeight};Element.setStyle(c,h);return g},getOffsetParent:b,cumulativeOffset:d,positionedOffset:f,
cumulativeScrollOffset:function(c){var h=0,g=0;do{h+=c.scrollTop||0;g+=c.scrollLeft||0;c=c.parentNode}while(c);return new Element.Offset(g,h)},viewportOffset:function(c){$(q);var h=0,g=0,l=document.body,q=c;do{h+=q.offsetTop||0;g+=q.offsetLeft||0;if(q.offsetParent==l&&Element.getStyle(q,"position")=="absolute")break}while(q=q.offsetParent);q=c;do if(q!=l){h-=q.scrollTop||0;g-=q.scrollLeft||0}while(q=q.parentNode);return new Element.Offset(g,h)},absolutize:function(c){c=$(c);if(Element.getStyle(c,
"position")==="absolute")return c;var h=b(c),g=c.viewportOffset();h=h.viewportOffset();g=g.relativeTo(h);h=c.getLayout();c.store("prototype_absolutize_original_styles",{left:c.getStyle("left"),top:c.getStyle("top"),width:c.getStyle("width"),height:c.getStyle("height")});c.setStyle({position:"absolute",top:g.top+"px",left:g.left+"px",width:h.get("width")+"px",height:h.get("height")+"px"});return c},relativize:function(c){c=$(c);if(Element.getStyle(c,"position")==="relative")return c;var h=c.retrieve("prototype_absolutize_original_styles");
h&&c.setStyle(h);return c}});"getBoundingClientRect"in document.documentElement&&Element.addMethods({viewportOffset:function(c){c=$(c);if(j(c))return new Element.Offset(0,0);c=c.getBoundingClientRect();var h=document.documentElement;return new Element.Offset(c.left-h.clientLeft,c.top-h.clientTop)}})})();window.$$=function(){var a=$A(arguments).join(", ");return Prototype.Selector.select(a,document)};
